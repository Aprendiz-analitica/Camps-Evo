...
var agentapi = new function () {
    ...
    this.ReadDTMFBuffer = function (callId, clearBuffer) {
        /// <summary>Gets the call DTMF buffer.</summary>
        /// <param name="callId" type="Number">callId (Optional). Gets the current call DTMF buffer if not set.</param>
        /// <param name="clearBuffer" type="Boolean">clearBuffer (Optional). Clears the call DTMF buffer if set to true.</param>
        /// <returns type="Deferred"></returns>
        var ErrMsgHead = "[ReadDTMFBuffer] ";
        var deferred = new Deferred();
        ...
        try {
            ...
            var webAgentCallBack = function (idCall) {
            
				var query;

				query = "callId=" + encodeURIComponent(callId) + "&clearBuffer=" + encodeURIComponent(clearBuffer);

				makeHttpRequest(webServiceActionURL("ReadDTMFBuffer", query))
				.done(function (response) {
					
					o2Ret = {};
                    o2Ret.idCall = response.callId;
                    o2Ret.dtmfs = response.dtmfs;
					
					console.log( "DTMFS RECEIVED[" + response.callId + "]: " + response.dtmfs);
					deferred.resolve(JSON.stringify(o2Ret));
				})
				.fail(function (code, text) {
					deferred.reject(code, text);
				});
