    ...
    <div class="container-fluid" style="width: 100%">            
        <table id="dataTable" class="table table-striped table-bordered" style="color: #f8db66">
            ...
            <tbody>
                <%
                    try {
                        ...
                        String sql1 = "SELECT "
                                ...
                                + "    B.idFinal, "
                                + "    F.DESCRIPCION, "
                                + "    (U.NOMBRE + ' ' + U.APELLIDO1) AS NOMBRE, "
                                + "    B.tCreacion AS FechaGestion, "
                                + "    B2.tCreacion AS FechaAbandono "
                                + "FROM "
                                + "    __Gestion_Abandonos A "
                                + "JOIN "
                                + "    TRANSACCION B ON B.idTransaccion = A.idTransactionGestion "
                                + "JOIN "
                                + "    TRANSACCION B2 ON B2.idTransaccion = A.idTransactionOriginal "
                                + "LEFT JOIN "
                                + "    FINALES F ON F.idFinal = B.idFinal AND F.IDCAMPANYA = B.idCampanya "
                                + "LEFT JOIN "
                                + "    USUARIO U ON U.IDUSUARIO = A.idAgente "
                                + "WHERE "
                                + "    B.tCreacion BETWEEN '" + fechaIn + "T00:00:00' AND '" + fechaFin + "T23:59:59' "
                                + "    AND B2.idCampanya = " + desfinal
                                + "UNION "
                                + "SELECT "
                                + "    A.*, "
                                + "    B.idFinal, "
                                ...
                                + "    __Gestion_Abandonos A "
                                + "JOIN "
                                + "    tbHistoFTransacciones B ON B.idTransaccion = A.idTransactionGestion "
                                + "JOIN "
                                + "    tbHistoFTransacciones B2 ON B2.idTransaccion = A.idTransactionOriginal "
                                + "LEFT JOIN "
                                + "    FINALES F ON F.idFinal = B.idFinal AND F.IDCAMPANYA = B.idCampanya "
                                + "LEFT JOIN "
                                + "    USUARIO U ON U.IDUSUARIO = A.idAgente "
                                + "WHERE "
                                + "    B.tCreacion BETWEEN '" + fechaIn + "T00:00:00' AND '" + fechaFin + "T23:59:59' "
                                + "    AND B.idCampanya = " + desfinal
                                + "UNION "
                                + "SELECT "
                                + "    B.idTransaccion AS idtransactionOriginal, "
                                + "    'Abierto' AS estado, "
                                + "    0 AS idtransactionGestion, "
                                + "    0 AS idAgente, "
                                + "    B.idFinal, "
                                + "    F.DESCRIPCION, "
                                + "    (U.NOMBRE + ' ' + U.APELLIDO1) AS NOMBRE, "
                                + "    NULL AS FechaGestion, "
                                + "    B.tCreacion AS FechaAbandono "
                                + "FROM "
                                + "    TRANSACCION B "
                                + "LEFT JOIN "
                                + "    FINALES F ON F.idFinal = B.idFinal AND F.IDCAMPANYA = B.idCampanya "
                                + "LEFT JOIN "
                                + "    USUARIO U ON U.IDUSUARIO = B.idAgente "
                                + "WHERE "
                                + "    B.tCreacion BETWEEN '" + fechaIn + "T00:00:00' AND '" + fechaFin + "T23:59:59' "
                                + "    AND B.idCampanya = " + desfinal
                                + "    AND B.idFinal = 10 "
                                + "    AND B.idTransaccion NOT IN (SELECT idtransactionOriginal FROM __Gestion_Abandonos) "
                                + "UNION "
                                + "SELECT "
                                + "    B.idTransaccion AS idtransactionOriginal, "
                                + "    'Abierto' AS estado, "
                ...
                %>
                <tr>
                    <td><%= rs1.getString("idTransactionOriginal")%></td>
                    <td><%= rs1.getString("estado")%></td>    
                    <td><%= rs1.getString("idTransactionGestion")%></td>    
                    <td><%= rs1.getString("idAgente")%></td>    
                    <td><%= rs1.getString("idFinal")%></td>    
                    <td><%= rs1.getString("Descripcion")%></td>    
                    <td><%= rs1.getString("Nombre")%></td>    
                    <td><%= rs1.getString("FechaAbandono")%></td>  
                    <td><%= rs1.getString("FechaGestion")%></td>                      
                </tr>
                <%
                        }
                        rs1.close();
