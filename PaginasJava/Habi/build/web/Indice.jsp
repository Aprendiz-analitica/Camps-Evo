        ...
        <table id="dataTable" class="table table-striped table-bordered" style="color: #f8db66">
            ...
            <thead>
                <tr style="font-weight: bold; font-family: Lato; color: white; background-color: #f37920; border : 1px solid #f37920;">  
                    <%if (desfinal != 100000523) { %>
                    <th>NIDNUEVOS</th>
                    <th>FECHACREACION</th>
                    <th>FECHAASIGNACION</th>
                    <th>CIUDAD</th>
                    <th>TELEFONO</th>
                    <th>NOMBREEDIFICIOCONJUNTO</th>
                    <th>ESTADOCLIENTE</th>
                    <th>IDREGISTRO</th>
                    <th>PRIORIDAD</th>
                    <th>BARRIDO</th>
            ...
            <tbody>
                <%
                    try {
                        ...
                        String sql1 = "SELECT c.IDORIGINAL, "
                                + "c.NOMBRE, "
                                + "c.TELEFONO, "
                                + "c.APELLIDO1, "
                                + "c.OBSERVACIONES, "
                                + "PivotTable.FECHACREACION, "
                                + "PivotTable.FECHAASIGNACION, "
                                + "PivotTable.ESTADOCLIENTE, "
                                + "PivotTable.BARRIDO, "
                                + "PivotTable.PRIORIDAD, "
                                + "PivotTable.IDREGISTRO, "
                                + "T.sDescripcionFinal AS DESCRIPCIONFINAL, "
                                + "t.tFinal AS TFINAL, "
                                + "CONCAT(age.sNombreUsuario, ' ', age.sApellido1Usuario) AS AGENTE "
                                + "FROM (SELECT clave, idSujeto, valor "
                                + "      FROM tbDatosCliente) AS SourceTable "
                                + "PIVOT (MAX(valor) FOR clave IN "
                                + "(FECHACREACION, FECHAASIGNACION, BARRIDO, PRIORIDAD, IDREGISTRO, ESTADOCLIENTE)) AS PivotTable "
                                ...
                                + "       (SELECT DESCRIPCION "
                                + "        FROM FINALES "
                                + "        WHERE idFinal = t.idFinal AND idCampanya = t.idCampanya) AS DESCRIPCIONFINAL, "
                                + "       t.tFinal AS TFINAL, "
                                + "       CONCAT(age.sNombreUsuario, ' ', age.sApellido1Usuario) AS AGENTE "
                                + "FROM (SELECT clave, idSujeto, valor "
                                + "      FROM tbDatosCliente) AS SourceTable "
                        ...
                        if (desfinal != 100000523) {
                            ...
                            ps1 = con1.prepareStatement(revicion);
                        }
                ...
                <tr>
                    ...
                    <td><%=rs1.getString("FECHACREACION")%></td>
                    <td><%=rs1.getString("FECHAASIGNACION")%></td>
                    <td><%=rs1.getString("NOMBRE")%></td>
                    <td><%=rs1.getString("TELEFONO")%></td>
                    <td><%=rs1.getString("APELLIDO1")%></td>
                    <td><%=rs1.getString("ESTADOCLIENTE")%></td>
